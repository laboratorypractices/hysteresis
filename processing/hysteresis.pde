...
void setup() {
  size(950,600);
  surface.setTitle("Laboratory practice - Hysteresis");
...
void draw() {
  ...
  // Títulos alineados
  textAlign(LEFT);
  textSize(22); fill(0,70,130); text("Laboratory practice",40,40);
  textSize(16); text("Hysteresis",40,60);

  // Etiquetas campos (10% más pequeñas)
  fill(0); textSize(11);
  text("Setpoint (°C)",40,95); 
  text("ΔT1 (°C)",40,145); 
  text("ΔT2 (°C)",40,195); 
  text("Data file",40,245);

  // Datos sensores
  textSize(15); fill(0);
  text("Ambient temperature: "+nf(Tamb,1,2)+" °C",320,110);
  text("Cell temperature: "+nf(Tsup,1,2)+" °C",320,140);
  ...
  if(isLogging){
    ...
    int min=elapsed/60; 
    int sec=elapsed%60;
    text("Test time: "+nf(min,2)+":"+nf(sec,2),320,170);
  }
  ...
  if(isLogging){ 
    ...
    text("Saving to: "+currentFileName,102,350);
  } else { 
    fill(150,0,0); textSize(11);
    text("Test stopped",80,350);
  }
}
...
void drawGraph(){
  ...
  fill(200,0,0); rect(gx+570, gy-70, 15,15); fill(0); text("Ambient temperature", gx+520, gy-58);
  fill(0,180,0); rect(gx+570, gy-50, 15,15); fill(0); text("Cell temperature", gx+510, gy-38);
  fill(0,0,180); rect(gx+570, gy-30, 15,15); fill(0); text("Setpoint temperature", gx+518, gy-18);
...
void toggleGuardar(){ 
  if(!isLogging){
    ...
    btnGuardar.setLabel("Start test");
  }
}
...
void toggleVentilador(){
  fanON=!fanON;
  if(fanON){ 
    btnVentilador.setLabel("Fan ON"); 
    btnVentilador.setColorBackground(color(0,180,0)); 
    myPort.write("FAN_ON\n");
  }
  else{ 
    btnVentilador.setLabel("Fan OFF"); 
...
void guardarGrafica(){
  ...
  saveFrame(nombreImg);
  println("Plot saved as "+nombreImg);
}
